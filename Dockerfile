...
ENV OTP_LENGTH=6
RUN go build
CMD ./discord-ec2-manager -t $BOT_TOKEN -c $CHANNEL_ID -i $INSTANCE_ID -sg $SECURITY_GROUP_ID -a $AMI_ID -sn $SUBNET_ID -u $PATH_TO_USERDATA -tk $TAG_KEY -tv $TAG_VALUE -svc $USER_SERVICE -p $USER_PORT -scp $SERVICE_CHECK_PORT -o $OTP_LENGTH